...
Hci.prototype.pollIsDevUp = function() {
  ...
  if (this._isDevUp !== isDevUp) {
    // since we are using the user socket rather than the raw socket, we no longer care if the device is up
    if (isDevUp || true) {
...
Hci.prototype.setActiveScanEnabled = function(enableActiveScan) {
  this._activeScan = enableActiveScan;
...
Hci.prototype.setScanParameters = function() {
  ...
  // data
  if (this._activeScan) {
    console.log('Configuring active scan')
    cmd.writeUInt8(0x01, 4); // type: 0 -> passive, 1 -> active
  } else {
    console.log('Configuring passive scan')
